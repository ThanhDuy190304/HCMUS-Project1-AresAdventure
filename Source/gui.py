...
def save_result_to_file(output_file, result, algorithm_name):
    final_state_path, num_checked_states, final_node, total_time, memory_usage = result
...
def run_algorithm(game, result_path, algorithm_name, result_window):
    ...
    if algorithm_name == "Breadth-First Search":
        ...
        is_algorithm_running = False
        run_button.config(state=tk.NORMAL)  # Bật lại nút "Run"
        return
...
def load_and_run_algorithm():
    ...
    input_path = f"input_{str(current_level).zfill(2)}.txt"
    output_path = f"output_{str(current_level).zfill(2)}.txt"
...
def load_level(level):
    ...
    current_level = level
    input_path = f"input_{str(level).zfill(2)}.txt"
