...
class Sokoban:
    ...
    def find_position_player(self):
        ...
        for x in range(len(self.board)):
            for y in range(len(self.board[0])):
                if self.board[x][y] == self.PLAYER_SYMBOL:
                    return (x,y)
    ...
    def is_stone_in_corner(self, stone_pos):
        ...
        left_wall = (y > 0 and self.board[x][y - 1] in self.WALLS)
        right_wall = (y < len(self.board[0]) - 1 and self.board[x][y + 1] in self.WALLS)
        
        # Check for walls above and below the stone position
        top_wall = (x > 0 and self.board[x - 1][y] in self.WALLS)
        bottom_wall = (x < len(self.board) - 1 and self.board[x + 1][y] in self.WALLS)
    ...
    def is_free_pos(self, positions):
        ...
        for x, y in positions:
            ...
            if self.board[x][y] in self.WALLS:
    ...
    def get_push_actions_above_stone(self, x, y):
        ...
        if x - 1 >= 0 and self.board[x - 1][y] == self.STONE_SYMBOL:
            # Push stone up
            if x - 2 >= 0 and self.board[x - 2][y] not in self.WALLS:
    ...
    def get_push_actions_below_stone(self, x, y):
        ...
        if x + 1 < len(self.board) and self.board[x + 1][y] == self.STONE_SYMBOL:
            # Push stone down
            if x + 2 < len(self.board) and self.board[x + 2][y] not in self.WALLS:
    ...
    def get_push_actions_left_stone(self, x, y):
        ...
        if y - 1 >= 0 and self.board[x][y - 1] == self.STONE_SYMBOL:
            # Push stone left
            if y - 2 >= 0 and self.board[x][y - 2] not in self.WALLS:
    ...
    def get_push_actions_right_stone(self, x, y):
        ...
        if y + 1 < len(self.board[0]) and self.board[x][y + 1] == self.STONE_SYMBOL:
            # Push stone right
            if y + 2 < len(self.board[0]) and self.board[x][y + 2] not in self.WALLS:
